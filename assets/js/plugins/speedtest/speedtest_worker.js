...
var settings = {
    ...
    job: 0,
    connection: 'ethernet',
    url_dl: 'https://speedtest.test.com/web/garbage', // path to a large file or garbage.php, used for download test. must be relative to this js file
    url_ul: 'https://speedtest.test.com/web/empty', // path to an empty file, used for upload test. must be relative to this js file
    url_ping: 'https://speedtest.test.com/web/empty', // path to an empty file, used for ping test. must be relative to this js file
    url_getIp: 'https://speedtest.test.com/web/getIp', // path to getIP.php relative to this js file, or a similar thing that outputs the client's ip
    ...
    url_telemetry: 'https://pfi-cloud.test.com/speedtest/telemetry' // path to the script that adds telemetry data to the database
}
...
this.addEventListener('message', function(e) {
    ...
    if (params[0] === 'start' && testStatus === 0) { // start new test
        ...
        try {
            ...
            if (typeof s.name !== 'undefined') settings.name = s.name; // name | username

            if (typeof s.mac !== 'undefined') settings.mac = s.mac; // mac address

            if (typeof s.connection !== 'undefined') settings.connection = s.connection; // connection type
        ...
        var runNextTest = function() {
            if (test_pointer >= settings.test_order.length) { testStatus = 4;
                sendTelemetry(); return; }
            switch (settings.test_order.charAt(test_pointer)) {
                case 'I':
                    { test_pointer++;getIp(runNextTest); }
                    break;
                case 'D':
                    { test_pointer++;testStatus = 1;dlTest(runNextTest); }
                    break;
                case 'U':
                    { test_pointer++;testStatus = 3;ulTest(runNextTest); }
                    break;
                case 'P':
                    { test_pointer++;testStatus = 2;pingTest(runNextTest); }
                    break;
                case '_':
                    { test_pointer++;setTimeout(runNextTest, 1000); }
                    break;
                default:
                    test_pointer++;
            }
...
function clearRequests() {
    ...
    if (xhr) {
        for (var i = 0; i < xhr.length; i++) {
            try { xhr[i].onprogress = null;
                xhr[i].onload = null;
                xhr[i].onerror = null } catch (e) {}
            try { xhr[i].upload.onprogress = null;
                xhr[i].upload.onload = null;
                xhr[i].upload.onerror = null } catch (e) {}
...
function dlTest(done) {
    ...
    var testStream = function(i, delay) {
        setTimeout(function() {
            ...
            // send xhr
            try { if (settings.xhr_dlUseBlob) xhr[i].responseType = 'blob';
                else xhr[i].responseType = 'arraybuffer' } catch (e) {}
    ...
    var doPing = function() {
        ...
        xhr[0].onload = function() {
            ...
            if (i === 0) {
                ...
                if (i === 1) ping = instspd; /* first ping, can't tell jitter yet*/
                else {
...
function sendTelemetry() {
    ...
    try {
        ...
        fd.append('mac', settings.mac)
        fd.append('connection', settings.connection)
        fd.append('name', settings.name)
        fd.append('job', settings.job)
        xhr.send(fd)
    } catch (ex) {
        var postData = 'dl=' + encodeURIComponent(dlStatus) + '&ul=' + encodeURIComponent(ulStatus) + '&ping=' + encodeURIComponent(pingStatus) + '&jitter=' + encodeURIComponent(jitterStatus) + '&log=' + encodeURIComponent(settings.telemetry_level > 1 ? log : '') +
            '&name=' + encodeURIComponent(settings.name) + '&mac=' + encodeURIComponent(settings.mac) + '&connection=' + encodeURIComponent(settings.connection) + '&job=' + encodeURIComponent(settings.job)
