...
pusher = Pusher(app_id=u'pfi', key=u'981847356', secret=u'asgaADFGA3451aSDFAGAGUIO456',ssl=False, host=u'pfi.test.com', port=4567)
...
def send_notification(url, data=None, headers=None):
    ...
    printer('%s %s' % (('GET', 'POST')[bool(data)], schemed_url),
            debug=True)

    return Request(schemed_url, data, headers)
        ...
        try:
            if (timeit.default_timer() - self.starttime) <= self.timeout:
                ...
                while (not SHUTDOWN_EVENT.isSet() and
                        ...
                        self.timeout):
                    self.result.append(len(f.read(1024)))
                    if self.result[-1] == 0:
                        break
                f.close()
        except IOError:
...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        #                        headers=headers)
        request = build_request('://pfi.test.com/assets/speedtest/custom-settings.xml',
                                headers=headers)       
    ...
    def set_mini_server(self, server):
        ...
        if e:
            raise SpeedtestMiniConnectFailure('Failed to connect to %s' %
                                              server)
            pusher.trigger(u'speedtest',u'error',{u'message': u'Failed to connect'})
        ...
        if not urlparts or not extension:
            raise InvalidSpeedtestMiniServer('Invalid Speedtest Server: '
                                             '%s' % server)
            
            pusher.trigger(u'speedtest',u'error',{u'message': u'Invalid speedtest server'})
        self.servers = [{
            'sponsor': 'Brandon Bailey',
            'name': urlparts[1],
            'd': 0,
    ...
    def get_best_server(self, servers=None):
        ...
        try:
            ...
            raise SpeedtestBestServerFailure('Unable to connect to servers to '
                                             'test latency.')
            pusher.trigger(u'speedtest',u'error',{u'message': u'Unable to connect to servers to test latency'})
...
def shell():
    ...
    pusher.trigger(u'speedtest',u'update',{u'message': u'Retrieving speedtest configuration'})
    try:
        speedtest = Speedtest()
    except (ConfigRetrievalError, HTTP_ERRORS):
        printer('Cannot retrieve speedtest configuration')
        pusher.trigger(u'speedtest',u'error',{u'message': u'Cannot retrieve speedtest configuration'})
    ...
    if args.list:
        try:
            ...
            pusher.trigger(u'speedtest',u'error',{u'message': u'Cannot retrieve speedtest server list'})
            raise SpeedtestCLIError(get_exception())
    ...
    if not args.mini:
        ...
        pusher.trigger(u'speedtest',u'update',{u'message': u'Selecting best server based on latency'})
    ...
    pusher.trigger(u'speedtest',u'update',{u'message': u'Testing Download Speed'})
    ...
    pusher.trigger(u'speedtest',u'download-speed',{u'message': u'%0.2f M%s' % ((results.download / 1000.0 / 1000.0) / args.units[1], args.units[0])})

    jsonResultsData ={}
    jsonResultsData["download"] = "%0.2f M%s/s" % ((results.download / 1000.0 / 1000.0) / args.units[1], args.units[0])

    printer('Testing upload speed', quiet,
            end=('', '\n')[bool(debug)])
    pusher.trigger(u'speedtest',u'update',{u'message': u'Testing Upload Speed'})
    ...
    pusher.trigger(u'speedtest',u'upload-speed',{u'message': u'%0.2f M%s' % ((results.upload / 1000.0 / 1000.0) / args.units[1], args.units[0])})

    # I added this to write to a file.
    # this is the line that outputs the information for use on the website.
    jsonResultsData["upload"] = "%0.2f M%s/s" % ((results.upload / 1000.0 / 1000.0) / args.units[1], args.units[0])


    f = open(LOG_FILE, "w")
    f.write(json.dumps(jsonResultsData))
    f.close() 
