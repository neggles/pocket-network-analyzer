...
rename_hostapd_ssid () {
        ...
        # Trim options so that only the file is saved
        CONF=$(echo $HOSTAPD_CONF | cut -d' ' -f2)
        # Remove the ':'
        CLEANMAC=$(echo $MAC | sed 's/\://g')
        # Pull the unique identifier from the MAC
        ENDMAC=$(expr substr $CLEANMAC 10 3)
        # Custom SSID
        SSID="Pocket-Fi-$ENDMAC"
        sed -i "s/^ssid=.*/ssid=$SSID/" $CONF
        echo "{\"ssid\":\"$SSID\"}" > /home/pfi/public_html/assets/ssid.conf
        return 0
}
