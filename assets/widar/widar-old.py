...
cellNumberRe = re.compile(r"^BSS\s+(?P<mac>.+)+\(on wlan0\)$")
regexps = [
    re.compile(r"^SSID: (?P<ssid>.*)$"),
    # re.compile(r"^Protocol:(?P<protocol>.+)$"),
    # re.compile(r"^Mode:(?P<mode>.+)$"),
    re.compile(r"^freq: (?P<frequency>[\d.]+)$"),
    re.compile(r"^signal: (?P<signal_strength>.+) (?P<signal_units>.+)$"),
]
...
def scan():
    ...
    cmd = ["/home/pfi/public_html/assets/widar/iw", args.interface, "scan"]
    while True:
    ...
    for line in lines:
        ...
        for expression in regexps:
            ...
            if result is not None:
                #network = result.groupdict()
                # if(checkSsidLength(network)):
                cells[-1].update(result.groupdict())
                continue
    return cells


def checkSsidLength(network):
...
def parse_args():
    ...
    description = (
        'Tool for running Brandon Bailey widar scanner'
    )
